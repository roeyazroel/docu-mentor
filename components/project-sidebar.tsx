...
export default function ProjectSidebar({ items, activeFileId, onFileSelect, onUpdateItems }: ProjectSidebarProps) {
  ...
  const startRenaming = (item: { id: string; name: string; type: "file" | "folder" }) => {
  ...
  const findItemById = (id: string, items: ProjectItem[]): ProjectItem | null => {
  ...
  const findParentById = (id: string, items: ProjectItem[], parent: ProjectItem | null = null): ProjectItem | null => {
    for (const item of items) {
      if (item.id === id) {
        return parent
      }
  ...
  const handleDragStart = (e: React.DragEvent, item: ProjectItem) => {
    ...
    dragImage.className = "bg-background border rounded p-2 shadow-md flex items-center gap-2 text-sm"
    ...
    if (draggedItem.type === "folder") {
      const isChild = (parent: FolderItem, childId: string): boolean => {
        ...
        for (const child of parent.children) {
          if (child.type === "folder" && isChild(child as FolderItem, childId)) {
            return true
          }
        }
        return false
      ...
      if (item.type === "folder" && isChild(draggedItem as FolderItem, item.id)) {
        setDragOverItemId(null)
        return
      }
    }
  ...
  const handleDrop = (e: React.DragEvent, targetItem: ProjectItem) => {
    ...
    if (dragPosition === "middle" && targetItem.type === "folder") {
      ...
      const insertAtPosition = (items: ProjectItem[], parentId: string | null): ProjectItem[] => {
        if (parentId === null) {
        ...
        return items.map((item) => {
          if (item.id === parentId && item.type === "folder") {
            const targetIndex = item.children.findIndex((child) => child.id === targetItem.id)
            if (targetIndex !== -1) {
      ...
      newItems = insertAtPosition(newItems, targetParent ? targetParent.id : null)
    }
  ...
  const getProviderIcon = (provider: StorageProvider) => {
    switch (provider) {
      ...
      default:
        return { icon: <HardDrive className="h-4 w-4" />, color: "text-gray-500" }
    }
  }
      ...
      >
        <ContextMenu>
          <ContextMenuTrigger>
            <div
              className={cn(
                ...
                isDragOver && dragPosition === "top" && "border-t-2 border-primary",
                isDragOver && dragPosition === "bottom" && "border-b-2 border-primary",
                isDragOver &&
                  dragPosition === "middle" &&
                  isFolder &&
            ...
            >
              ...
              {item.provider !== "local" && (
                <Badge variant="outline" className="ml-2 px-1 py-0 text-[10px]">
                  {item.provider === "google-drive" ? "Drive" : item.provider === "dropbox" ? "Dropbox" : "Box"}
                </Badge>
              )}
          ...
          <ContextMenuContent className="w-48">
            {isFolder && (
              <ContextMenuItem onClick={() => handleNewItem("file", item.id)} className="flex items-center">
                <FilePlus className="h-4 w-4 mr-2" />
                New File
              </ContextMenuItem>
            )}
            {isFolder && (
              <ContextMenuItem onClick={() => handleNewItem("folder", item.id)} className="flex items-center">
                <FolderPlus className="h-4 w-4 mr-2" />
                New Folder
              </ContextMenuItem>
            )}
            <ContextMenuItem
              onClick={() => startRenaming({ id: item.id, name: item.name, type: item.type })}
  ...
  return (
    ...
    >
      <div className="p-2 border-b">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as StorageProvider)}>
      ...
      {activeTab !== "local" && !connectedProviders[activeTab] && (
        <div className="p-4 text-center">
          <p className="text-sm text-muted-foreground mb-2">
            Connect to {activeTab === "google-drive" ? "Google Drive" : activeTab === "dropbox" ? "Dropbox" : "Box"}
            to access your files
          </p>
          <Button size="sm" onClick={() => handleConnectProvider(activeTab)} className="w-full">
      ...
      <Dialog open={newItemDialogOpen} onOpenChange={setNewItemDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{newItemType === "file" ? "Create New File" : "Create New Folder"}</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <Input
              value={newItemName}
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground mb-4">
              You'll be redirected to authenticate with your account. This is a demo, so no actual authentication will
              occur.
            </p>
